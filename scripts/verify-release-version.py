#!/usr/bin/env python3
"""Verify qCoder source, public-install pins, and optional artifact versions."""
...
def release_metadata(source_root: Path) -> dict[str, object]:
    ...
    if not isinstance(data["source_version"], str):
        raise ValueError("release_metadata_source_version_invalid")
    if not isinstance(data["source_posture"], str):
        raise ValueError("release_metadata_posture_invalid")
    if not isinstance(data["current_public_version"], str):
        raise ValueError("release_metadata_public_version_invalid")
...
def verify_release_version(
    *,
    ...
    if source_posture != UNPUBLISHED_CANDIDATE:
        raise ValueError("release_posture_not_implemented")
