...
def test_contract_surface_is_additive_and_inventory_is_unchanged() -> None:
    ...
    assert contract_digest == ("21c7ff5923a7e58bcd881a75ebd83947dcfae080afcbf8d6608f990e82136252")
...
def test_every_checkpoint_result_is_deterministically_actionable(tmp_path: Path) -> None:
    ...
    for checkpoint_kind in CHECKPOINT_KINDS:
        ...
        assert result["schema_id"] == "qcoder.current_loop.coordinator_result.v21"
...
def test_current_loop_cli_intent_review_confirmation_sequence_is_actionable(
    ...
    assert proposed["first_value_dialogue"]["customer_actions"] == [
        "Use recommended choices",
        "Review or change choices",
    ]
    assert len(proposed["first_value_dialogue"]["initial_decision_groups"]) <= 3
    assert (
        proposed["first_value_dialogue"]["explicit_confirmation_required_before_source_generation"]
        is True
    )
    assert proposed["first_value_dialogue"]["routine_procedural_narration"] is False
