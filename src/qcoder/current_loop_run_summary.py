...
def build_run_summary(
    *,
    ...
    reconciliation = deepcopy(
        dict(
            ...
            or {
                ...
                "relationships": [],
                "limitations": [
                    "No explicit causal reconciliation was supplied; this result is not current."
                ],
            }
        )
    )
    ...
    summary = {
        ...
        "freshness": {
            "status": "fresh" if current_run else "stale",
            "stale_reasons": (
                []
                if current_run
                else list(reconciliation.get("currentness_reasons", ["causal_lineage_missing"]))
            ),
...
def run_summary_contract_snapshot() -> dict[str, Any]:
    payload = {
        ...
        "current_run_requires_validated_causal_reconciliation": True,
        "independent_latest_role_heads_establish_currentness": False,
        "bare_counts_establish_currentness": False,
    }
