...
def _unsafe_projection_text(value: str) -> bool:
    ...
    if re.search(r"!?\[[^\]]+\]\([^)]*\)", value):
        return True
    if re.search(
        r"<\s*/?\s*(?:script|style|iframe|object|embed|button|form|a)\b", value, re.IGNORECASE
    ):
        return True
...
def _assistant_values_contradict_authority(values: Sequence[str], axes: Mapping[str, str]) -> bool:
    for value in values:
        ...
        if re.search(
            ...
            return True
        if (
            re.search(r"\b(?:execution|simulation)\b.{0,20}\b(?:authorized|permitted)\b", text)
            and "not authorized" not in text
            and "separate authorization" not in text
        ):
            return True
...
def canonical_first_value_delivery(
    value: Mapping[str, Any], *, review_revision_value: str
) -> dict[str, Any]:
    ...
    validated = validate_first_value(value)
    if not isinstance(review_revision_value, str) or not review_revision_value.startswith(
        "review-revision-"
    ):
...
def validate_canonical_first_value_delivery(
    value: Mapping[str, Any], *, review_revision_value: str
) -> dict[str, Any]:
    """Fail closed unless every canonical delivery representation agrees."""

    if not isinstance(value, Mapping):
        raise ReviewBeforeGenerationError("review_projection_delivery_invalid")
    expected = canonical_first_value_delivery(
        value.get("machine_projection", {}), review_revision_value=review_revision_value
    )
    if dict(value) != expected:
        raise ReviewBeforeGenerationError("review_projection_delivery_mismatch")
        ...
        "properties": {
            ...
            "implementation_recommendations": {
                ...
                "items": {
                    "type": "object",
                    "properties": {
                        "label": {**plain_text, "maxLength": 160},
                        "value": plain_text,
                    },
